...
if (isset($_POST['tabledata'])) {
    ...
    $result = mysqli_query($connection, "SELECT c.*,
                                            (SELECT IFNULL(SUM(s.total),0) FROM sales as s WHERE s.status=1 and s.customerid = c.id) as salesTotal,
                                            (SELECT IFNULL(SUM(r.amount),0) FROM payment_received as r WHERE r.status=1 and r.customerid = c.id ) as receivedTotal
                                            FROM `customer` as c
                                            GROUP BY c.id 
                                        ");
...
if (isset($_POST['Add'])) {
    ...
    $res = mysqli_query($connection, "INSERT INTO `customer`(`name`, `mobile`, `email`, `details`, `address`, `pending`, `status`)
